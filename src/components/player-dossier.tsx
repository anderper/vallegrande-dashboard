    ...
    <div className="w-full max-w-4xl max-h-[94dvh] bg-slate-950 border border-slate-700 rounded-2xl flex flex-col shadow-2xl">
      ...
      <div className="overflow-y-auto p-4 sm:p-6 space-y-6">
        {editing ? <><button className="text-sm text-brand-400 underline" onClick={() => setEditing(false)}>Volver al resumen</button><RegistrationForm initial={player} admin onSaved={saved => { onUpdated(saved); setStatus(statusOf(saved)); setEditing(false); setPdfUrl(''); setMessage('Ficha guardada.'); }} /></> : <>
          ...
          <section className="glass-card p-5 space-y-4"><h3 className="font-bold">Expediente para la liga</h3><p className="text-sm text-slate-400">Ficha, foto recortada y firma del jugador, con su cédula por ambos lados. Adultos: certificado de antecedentes. Menores: cédula del apoderado por ambos lados y su autorización firmada, sin antecedentes.</p><button onClick={() => void generate()} disabled={busy || missing.length > 0} className="btn-primary w-full sm:w-auto disabled:opacity-50">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}Generar ficha PDF</button>{missing.length > 0 && <p className="text-xs text-amber-300">Completa los requisitos pendientes para generar el expediente.</p>}
